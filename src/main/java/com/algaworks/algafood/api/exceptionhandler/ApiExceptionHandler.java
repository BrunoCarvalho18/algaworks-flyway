...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		List<Problema.Object> problemObjects = bindingResult.getFieldErrors().stream()
				.map(objectError -> {
			String message = messageSource.getMessage(objectError, LocaleContextHolder.getLocale());
			...
			return Problema.Object
					.builder()
					.name(objectError.getField())
					.userMessage(message).build();
		}).collect(Collectors.toList());

		Problema problem = createProblemBuilder(status, problemType, detail).userMessage(detail).objects(problemObjects)
				.build();
